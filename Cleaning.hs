...
voteProcess :: String -> Int
---- a = "1"; a = "2"; a = "3" ; a = "4" ; a = "5" and else = 0
voteProcess :: a | a == "1" = 1| a == "2" = 2 | a == "3" = 3 | a == "4" | a == "5" = 5| otherwise = 0

theVote :: [[Int]]
theVote = map (map voteProcess) (drop 1 (map (drop 2) dirtyVotes))
...
cleanVotes = filter(/=[]) $ map removeBlankVotes $ map checkPreOrder (map checkDuplicates sort_Votes)

sort_Votes :: [[(String, Int)]]
sort_Votes = filter (/=[]) $ map sorTupAscending $ map removeBlankVotes $ map (zip candidates) (votes)

weigth_Stdvotes :: [([String], Int)]
weigth_Stdvotes = map (\xs -> (f x | x <- xs), weight)) cleanVotes

sortedVotes :: [[String]]
sort_Votes = map(\xs -> [f x | x <- xs] cleanVotes
